...
//    init_runtime();
    ...
    memset(_input_attrs, 0, _n_input * sizeof(rknn_tensor_attr));    // 初始化函数 memory set 作用是将某一块内存中的全部设置为指定的值，即将_input_attrs的值初始化为0
    for (uint32_t i = 0; i < _n_input; i++) {
        _input_attrs[i].index = i;
        // query info
        ret = rknn_query(ctx, RKNN_QUERY_INPUT_ATTR, &(_input_attrs[i]), sizeof(rknn_tensor_attr)); // 通过rknn模型的输入索引设置输入信息
        if (ret < 0) {
            printf("rknn_init error! ret=%d\n", ret);
            exit(-1);
        }
        dump_tensor_attr(&_input_attrs[i]); // 读取rknn模型输入属性
    }
...
//    rknn_tensor_format input_layout = RKNN_TENSOR_NCHW; // default fmt is NHWC, npu only support NHWC in zero copy mode
    ...
    _input_mems[0] = rknn_create_mem(ctx, _input_attrs[0].size_with_stride); //创建输入内存

    // rknn outputs
    printf("output tensors:\n");
    std::cout <<"sizeof(rknn_tensor_attr: " << sizeof(rknn_tensor_attr) << std::endl;
    memset(_output_attrs, 0, _n_output * sizeof(rknn_tensor_attr)); // 初始化操作

    for (uint32_t i = 0; i < _n_output; i++)  // 打印输出的属性
    {
    ...
    for (uint32_t i = 0; i < _n_output; i++) {
        ...
        // query info
        ret = rknn_query(ctx, RKNN_QUERY_OUTPUT_ATTR, &(_output_attrs[i]), sizeof(rknn_tensor_attr)); // 输入的值赋给模型的输入
        if (ret != RKNN_SUCC) {
            printf("rknn_query fail! ret=%d\n", ret);
            exit(-1);
        }
        dump_tensor_attr(&_output_attrs[i]); // 读取rknn模型输出属性
    }
    ...
    int h = image_in.rows;
    int w = image_in.cols;
    int h_ = std::ceil(h / 32.0) * 32;   // 计算调整后的高度
    int w_ = std::ceil(w / 32.0) * 32;   // 计算调整后的宽度
    if (h % 32 != 0) {
        h_ = h_;
    }
    else {
    ...
    cv::Mat padded;
    // 常量法扩充图像边界，常量 = 0
    cv::copyMakeBorder(image_in, padded, 0, h_ - h, 0, w_ - w, cv::BORDER_CONSTANT, cv::Scalar::all(0));
...
//    cv::resize(image_in, dst, cv::Size(img_w, img_h));
    ...
    using TYPE = uint8_t;
//    cv::cvtColor(dst, dst, cv::COLOR_BGR2GRAY);   // 已经在rknn中转换过了
    ...
    memcpy(_input_mems[0]->virt_addr, padded.data, width*_input_attrs[0].dims[1]*_input_attrs[0].dims[3]); // 640*416*3
